...
const corsOptions = {
  origin: (origin: string | undefined, callback: (err: Error | null, allow?: boolean) => void) => {
    // 개발 환경: localhost + ALLOWED_ORIGINS 허용
    if (process.env.APP_ENV === 'development') {
      // localhost 패턴 허용
      if (!origin || /^https?:\/\/(localhost|127\.0\.0\.1|0\.0\.0\.0)(:\d+)?$/.test(origin)) {
        console.log(`✅ CORS 허용 (개발-localhost): ${origin || 'no-origin'}`);
        return callback(null, true);
      }
      // ALLOWED_ORIGINS 리스트 허용
      if (allowedOrigins.includes(origin!)) {
        console.log(`✅ CORS 허용 (개발-allowlist): ${origin}`);
        return callback(null, true);
      }
      console.warn(`❌ CORS 차단 (개발): ${origin}`);
      return callback(new Error('Not allowed by CORS'), false);
    }
    
    // 운영 환경: ALLOWED_ORIGINS만 허용 (!origin 완전 차단)
    if (allowedOrigins.includes(origin!)) {
      console.log(`✅ CORS 허용 (운영): ${origin}`);
      callback(null, true);
    } else {
      console.warn(`❌ CORS 차단 (운영): ${origin || 'no-origin'}`);
...
// ─────────────────────────────
if (false) {
  // Vite 미들웨어 (HMR) - 임시로 비활성화 (연결 문제로 인해)
...
async function main() {
  ...
  console.log("DB_URL (masked):", maskDbUrl(process.env.DATABASE_URL));
